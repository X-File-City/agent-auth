...
use kanoniv_agent_auth::{ActionType, AgentKeyPair, Caveat, Delegation, Invocation, ProvenanceEntry, SignedMessage};
...
fn main() {
    ...
    let pass_result =
        kanoniv_agent_auth::verify_invocation(&pass_invocation, &delegate_identity, &identity).unwrap();
    ...
    let sub_pass_result =
        kanoniv_agent_auth::verify_invocation(&sub_pass_invocation, &sub_identity, &identity).unwrap();
